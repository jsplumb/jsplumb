    ...
    Connectors :
    {
        ...
        BEZIER : {
            ...
            _findControlPoint :function(point, anchor1Position, anchor2Position, anchor1, anchor2) {
                var p = [];            
                var majorAnchor = jsPlumb.Connectors.BEZIER.majorAnchor;
                var minorAnchor = jsPlumb.Connectors.BEZIER.minorAnchor;
                if (anchor1.orientation[0] == 0) {  // X
                	// todo parameterize hardcoded values here
                    var diff = anchor1Position[0] < anchor2Position[0] ? point[0] + minorAnchor : point[0] - minorAnchor;
                    p.push(diff);
                }
             // todo parameterize hardcoded values here
                else p.push(point[0] - (majorAnchor * anchor1.orientation[0]));

                if (anchor1.orientation[1] == 0) { // Y
                	//todo parameterize hardcoded values here
                    var diff = anchor1Position[1] < anchor2Position[1] ? point[1] + minorAnchor : point[1] - minorAnchor;
                    p.push(diff);
                }
             // todo parameterize hardcoded values here
                else p.push(point[1] + (majorAnchor * anchor2.orientation[1]));
            ...
            paint : function(d, ctx) {
	            ...
	            // gradient experiment.
	            // todo: how do we integrate gradients/patterns with the API?
	            // (this works and looks cool). 
	            /*var g = ctx.createLinearGradient(d[4],d[5],d[6], d[7]);
	            g.addColorStop(0,'#5eff84');  
	            g.addColorStop(1,'#ffd65e');  
	            ctx.strokeStyle = g;*/
