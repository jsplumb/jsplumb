 ...
 *
 * Update to Bezier control point function
 * Copyright (c) 2013 - CRCSI (http://crcsi.com.au/)
 * 
 * http://jsplumb.org
    ...
    */
    ...
    jsPlumb.Connectors.Bezier = function(params) {
    	...
    	var self = this,
            ...
            stub = params.stub || 20,
            majorAnchor = params.curviness || 0.3;

        this.type = "Bezier";	
        this.getCurviness = function() { return majorAnchor; };	

        this._findControlPoint = function(point, dims, targetEndpoint) {
            var too = targetEndpoint.anchor.getOrientation(targetEndpoint);
            var len = Math.sqrt(dims[0] * dims[0] + dims[1] * dims[1]);
            var lenToo = Math.sqrt(too[0] * too[0] + too[1] * too[1]);
            if (lenToo > 0.001)
                len /= lenToo;
            var dynVec = [majorAnchor * len * too[0],
                          majorAnchor * len * too[1]];
            var statVec = [stub * too[0],
                           stub * too[1]];
            return [point[0] + (dynVec[0] + statVec[0]),
                    point[1] + (dynVec[1] + statVec[1])];
        };
        ...
        this._compute = function(paintInfo, p) {                                
			var sp = p.sourcePos,
                ...
                _tx = sp[0] < tp[0] ? 0 : _w,
                _ty = sp[1] < tp[1] ? 0 : _h,
                _CP = self._findControlPoint([_sx, _sy], [_w, _h], p.targetEndpoint),
                _CP2 = self._findControlPoint([_tx, _ty], [_w, _h], p.sourceEndpoint);
