    ...
    var Drag = function(el, params, css, scope) {
        ...
        this.toGrid = function(pos) {
            if (this.params.grid == null) {
                ...
                var _dx = Math.floor(pos[0] / this.params.grid[0]),
                    _dxl = this.params.grid[0] * _dx,
                    _dxt = _dxl + this.params.grid[0],
                    _x = Math.abs(pos[0] - _dxl) <= snapThreshold ? _dxl : Math.abs(_dxt - pos[0]) <= snapThreshold ? _dxt : pos[0];

                var _dy = Math.floor(pos[1] / this.params.grid[1]),
                    _dyl = this.params.grid[1] * _dy,
                    _dyt = _dyl + this.params.grid[1],
                    _y = Math.abs(pos[1] - _dyl) <= snapThreshold ? _dyl : Math.abs(_dyt - pos[1]) <= snapThreshold ? _dyt : pos[1];
