<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="utf-8">
    <title>jsPlumb API Documentation</title>
    
    <script src="scripts/prettify/prettify.js"> </script>
    <script src="scripts/prettify/lang-css.js"> </script>
    <!--[if lt IE 9]>
      <script src="//html5shiv.googlecode.com/svn/trunk/html5.js"></script>
    <![endif]-->
    <link type="text/css" rel="stylesheet" href="styles/prettify-tomorrow.css">
    <link type="text/css" rel="stylesheet" href="styles/jsdoc-default.css">
    <?js if (env.conf.css) { ?>
        <?js for (var i in env.conf.css) { ?>
    <link type="text/css" rel="stylesheet" href="<?js= env.conf.css[i] ?>">
        <?js } ?>
    <?js } ?>
    <link rel="stylesheet" href="../demo/demo-all.css">
    <link rel="stylesheet" href="../docs/gollum-template.css">
    
</head>

<body>        
    <div id="headerWrapper">
        <div id="header">
            <div class="menu">
                <a href="../docs/home">Documentation</a>&nbsp;|&nbsp;
                <a href="../demo/">Demos</a>&nbsp;|&nbsp;
                <a href="../apidocs">API docs</a>&nbsp;|&nbsp;
                <a href="../tests/all-tests.html">Tests</a>&nbsp;|&nbsp;
                <a href="mailto:hello@jsplumbtoolkit.com">Contact</a>&nbsp;|&nbsp;
                <a href="http://github.com/sporritt/jsPlumb/">GitHub</a>&nbsp;|&nbsp;
                <a href="https://groups.google.com/forum/?fromgroups#!forum/jsplumb">Discussion Group</a>&nbsp;|&nbsp;
                <a href="http://github.com/sporritt/jsPlumb/issues">Issues</a>
            </div>
        </div>
    </div>  

    <div id="main" class="apidoc-main">    
        <?js= content ?>
    </div>

    <nav>
        <?js= this.nav ?>
    </nav>

    <br clear="both">

    <script> prettyPrint(); </script>
    <script src="scripts/linenumber.js"> </script>
</body>
</html>
