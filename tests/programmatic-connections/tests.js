...
var testSuite = function () {
    ...
    //
    // this is new in 5.x, and kind of replaces what `makeSource` used to do: here we are testing that if we ask jsplumb to
    // connect from some div, that it will extract values from that div to use on the source endpoint of the connection. in 2.x and in
    // 4.x this was done by looking for a source definition registered to the element.
    //
    test("extract endpoint/anchor spec from source element on `connect` call", function() {
        var d1 = support.addDiv("d1"), d2 = support.addDiv("d2")
        d1.setAttribute("endpointType", "Rectangle")
        _jsPlumb.importDefaults({
            parameterExtractor:function(element, endpointIndex) {
                if (endpointIndex === 0) {
                    return {
                        endpoint: element.getAttribute("endpointType"),
                        anchor: "Continuous",
                        paintStyle:{fill:"BLACK"},
                        hoverPaintStyle:{fill:"BRONZE"}
                    }
                }
            }
        })
        ...
        equal(c.endpoints[0].anchor.isContinuous, true, "anchor was set from parameter extractor")
        ...
        equal(c.endpoints[0].hoverPaintStyle.fill, "BRONZE", "endpoint hover paint style was set from parameter extractor")
    })

    test("extract endpoint/anchor spec from target element on `connect` call", function() {
        var d1 = support.addDiv("d1"), d2 = support.addDiv("d2")
        d2.setAttribute("endpointType", "Blank")
        _jsPlumb.importDefaults({
            parameterExtractor:function(element, endpointIndex) {
                if (endpointIndex === 1) {
                    return {
        ...
        var c = _jsPlumb.connect({source: d1, target: d2});
        equal(c.endpoints[1].anchor.isContinuous, true, "anchor was set from parameter extractor")
        equal(c.endpoints[1].endpoint.type, "Blank", "endpoint type was set from parameter extractor")
    })

    test("extract endpoint/anchor spec from source and target element on `connect` call", function() {
        var d1 = support.addDiv("d1"), d2 = support.addDiv("d2")
        d1.setAttribute("endpointType", "Rectangle")
        d2.setAttribute("endpointType", "Blank")
        _jsPlumb.importDefaults({
        ...
        equal(c.endpoints[1].anchor.isContinuous, true, "anchor was set from parameter extractor")
        equal(c.endpoints[1].endpoint.type, "Blank", "endpoint type was set from parameter extractor")
        equal(c.endpoints[0].anchor.type, "Center", "anchor was set from parameter extractor")
        equal(c.endpoints[0].endpoint.type, "Rectangle", "endpoint type was set from parameter extractor")
    })
