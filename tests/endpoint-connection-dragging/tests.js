...
// ----------------------------------------------------------------------------------------------------------------
    ...
    test("endpoint:click but not drag results in drag proxy being cleaned up", function() {
        ...
        equal(2, document.querySelectorAll("[data-jtk-vertex-id]").length, 2, "two managed elements after init");

        _jsPlumb.trigger(ec1, "mousedown", support.makeEvent(ec1));
        _jsPlumb.trigger(ec1, "mouseup", support.makeEvent(ec1));

        equal(2, document.querySelectorAll("[data-jtk-vertex-id]").length, 2, "two managed elements after aborted drag: drag element was cleaned up.");
    });
